...
def measure_mottle(path_img, image, thresh):
    ...
    lower = np.array([65,0,0])          # HSV下限
    upper = np.array([160,255,255])     # HSV上限
...
def calc(folder_path):
    ...
    exts_list = sum([[ext.lower(), ext.upper()] for ext in exts], [])
    files = []
    ...
    for file in files:
        ...
        d = Detect()
        d.set_param(bin_thr=30)
        img, main_obj = d.extr_leaf(file)
        whole_area = cv2.contourArea(main_obj)

        lower = np.array([80,0,0])          # HSV下限
        upper = np.array([160,255,255])     # HSV上限

        img_hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)

        bin_green = cv2.inRange(img_hsv, lower, upper)
        green_area = cv2.countNonZero(bin_green)
        conts = cv2.findContours(bin_green, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)[0]
        cv2.drawContours(img, conts, -1, color=(0,0,255), thickness=3)
        mottle_area = whole_area - green_area
        ...
        if type(mottle_area) == str:
            ...
            ratio_mottle = green_area / whole_area * 100

        data = [img_name, whole_area, mottle_area, green_area, ratio_mottle]
